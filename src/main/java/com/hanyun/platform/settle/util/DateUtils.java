...
public class DateUtils {
     ...
     * @param start 开始日期
     * @param end 结束日期
     * @return 季度起始时间列表：[[m1start,m1end],[m2start,m2end],....]
     */
    public static List<Date[]> splitInSeason(Date start, Date end){
        List<Date[]> seasonlst = new ArrayList<Date[]>();
        ...
        LocalDate tmpseasonstart = ldstart;//季度开始时间
        LocalDate tmpseasonend = LocalDate.fromDateFields(getLastDateOfSeason(ldstart.toDate())).dayOfMonth().withMaximumValue();

        while(true){
            if(! ldend.isAfter(tmpseasonend)){
                seasonlst.add(new Date[]{
                        tmpseasonstart.toDate(),
                        ldend.toDateTimeAtCurrentTime().millisOfDay().withMaximumValue().toDate()});
                break;
            }
            seasonlst.add(new Date[]{
                    tmpseasonstart.toDate(),
                    tmpseasonend.toDateTimeAtCurrentTime().millisOfDay().withMaximumValue().toDate()});
            tmpseasonstart = tmpseasonend.plusDays(1);
            //tmpseasonend = tmpseasonstart.dayOfMonth().withMaximumValue();
            tmpseasonend = LocalDate.fromDateFields(getLastDateOfSeason(tmpseasonstart.toDate())).dayOfMonth().withMaximumValue();
...
/*        String strDate = "2013-01-01";
        ...
        //String str = "2018-02-03";
/*        Date da1 = new Date();
        String date1 = DateFormatUtil.formatDate(da1);
        Date date2 =  parseDate(date1,YYYYMMDD);
        System.out.println(getSeasonDate(date2));
        System.out.println(getLastDateOfSeason(new Date()));*/

        Date da1 = new Date();
        Thread.sleep(1000);
        Date da2 = new Date();
        List<Date[]> list =  splitInSeason(da1,da2);
    }
}  